                ...
                <div class="col-md-9">
                    ...
                    <button type="button" disabled class="btn btn-primary btn-flat"><span
                        ...
                        <div class="card-body p-0">
                                ...
                                <div class="row">
                                    <div class="col-md-12 card-header">
                                        <h3 class="card-title pull-left">Showing
                                            <strong>{{ count($latestMails) }}</strong>
                                            records
                                            of <strong>{{ $latestMails->total() }}</strong></h3>
                            ...
                            <div class="table-responsive mailbox-messages">
                                <table class="table table-hover table-striped mailbox">
                                    <tbody>
                                    @foreach($latestMails as $latestMail)
                                        <tr>
                                            <td class="mailbox-star">
                                                <a href="{{ route('admin.read.mailbox',[ 'id' => $latestMail->id]) }}"><i
                                                        class="fa fa-envelope text-danger"></i></a>
                                            </td>
                                            <td class="mailbox-name"><a
                                                    href="{{ route('admin.read.mailbox',[ 'id' => $latestMail->id]) }}"><b>{{ $latestMail->subject }}</b></a>
                                            </td>
                                            <td class="mailbox-subject">{{ substr($latestMail->description, 0, 150) }}@if(strlen($latestMail->description) > 150)
                                                    ...@endif
                                            </td>
                                            <td class="mailbox-date">{{ \App\Http\Controllers\SystemController::elapsedTime($latestMail->created_at) }}</td>
                                        </tr>
                                    @endforeach
                                    </tbody>
                                </table>
                        ...
                        <div class="card-footer p-0">
                            ...
                            <div class="row">
                                <div class="col-md-12 card-header">
                                    <h3 class="card-title pull-left">Showing <strong>{{ count($latestMails) }}</strong>
                                        records
                                        of <strong>{{ $latestMails->total() }}</strong></h3>
                                    <ul class="pagination pagination-sm m-0 float-right">
                                        {{ $latestMails->links() }}
                ...
                <div class="col-md-9">
                    <div class="card card-primary card-outline">
                        <center>
                            ...
                            <br>
                            <br>
                            <h4 class="text-danger">No Latest Notifications Were Found</h4>
                            <h6>
                                <hr>
